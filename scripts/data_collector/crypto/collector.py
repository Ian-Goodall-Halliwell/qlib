...
class CryptoNormalize(BaseNormalize):
    ...
    def normalize_crypto(
        ...
        print(df[symbol_field_name][1], 'remung')
        return df.reset_index()

    def normalize(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            df = self.normalize_crypto(df, self._calendar_list, self._date_field_name, self._symbol_field_name)
            print(df[self._symbol_field_name][1])
        except:
            df.columns = ['date','open','high','low','close','volume','symbol','QAV','numberoftrades','takerbuyBAV','takerbuyQAV','factor']
            df = self.normalize_crypto(df, self._calendar_list, self._date_field_name, self._symbol_field_name)
            print(df[self._symbol_field_name][1], 'mungled')
